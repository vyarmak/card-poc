DROP PROCEDURE IF EXISTS TransactionRefund;

DELIMITER //

CREATE PROCEDURE TransactionRefund(
...
BEGIN
  ...
  DECLARE PAR_amountToRefund DECIMAL(10,2);
  ...
  START TRANSACTION;
    SELECT idTransaction, idCard, amountCaptured INTO PAR_transactionFound, PAR_idCard, PAR_amountToRefund 
      FROM Transaction 
    ...
    END IF;
    IF (PAR_amountToRefund < IN_amount) THEN
      SELECT 'InvalidRefundAmount' INTO errCode;
      SIGNAL SQLSTATE 'SYSER'
        SET MESSAGE_TEXT = 'Invalid refund amount';
    END IF;
    UPDATE Transaction
      SET amount = amount - IN_amount, amountCaptured = amountCaptured - IN_amount
    WHERE idTransaction = IN_idTransaction;
    INSERT INTO TransactionLedger 
      (idTransactionLedger, idTransaction, createdAt, type, amount)
    VALUES
      (NULL, IN_idTransaction, NOW(), 'R', IN_amount);
